...
const HeroSection = () => {
  ...
  // Scroll progress specifically for the DNA animation
  const { scrollYProgress: dnaProgress } = useScroll({
    target: panelsRef,
    offset: ["start 80%", "start 20%"]
  });

  // Spring physics for smooth, bouncy animations
  const springConfig = { stiffness: 100, damping: 20, mass: 1 };
  const smoothProgress = useSpring(dnaProgress, springConfig);

  // DNA Spiral transformations for left panel (Student)
  const leftRotateY = useTransform(smoothProgress, [0, 0.5, 1], [45, -15, 0]);
  const leftRotateZ = useTransform(smoothProgress, [0, 0.5, 1], [-8, 4, 0]);
  const leftTranslateX = useTransform(smoothProgress, [0, 0.5, 1], [100, -20, 0]);
  const leftTranslateZ = useTransform(smoothProgress, [0, 0.5, 1], [-100, 50, 0]);
  const leftScale = useTransform(smoothProgress, [0, 0.5, 1], [0.85, 1.02, 1]);

  // DNA Spiral transformations for right panel (University)
  const rightRotateY = useTransform(smoothProgress, [0, 0.5, 1], [-45, 15, 0]);
  const rightRotateZ = useTransform(smoothProgress, [0, 0.5, 1], [8, -4, 0]);
  const rightTranslateX = useTransform(smoothProgress, [0, 0.5, 1], [-100, 20, 0]);
  const rightTranslateZ = useTransform(smoothProgress, [0, 0.5, 1], [-100, 50, 0]);
  const rightScale = useTransform(smoothProgress, [0, 0.5, 1], [0.85, 1.02, 1]);

  // Shared opacity and vertical movement
  const panelOpacity = useTransform(smoothProgress, [0, 0.3, 1], [0.4, 0.9, 1]);
  const panelY = useTransform(smoothProgress, [0, 1], [60, 0]);

  // Connection line animation (the DNA "backbone")
  const lineOpacity = useTransform(smoothProgress, [0, 0.5, 0.9, 1], [0.8, 0.6, 0.3, 0]);
  const lineScale = useTransform(smoothProgress, [0, 1], [1.2, 0]);

  // Overall container perspective shift
  const containerRotateX = useTransform(smoothProgress, [0, 1], [8, 0]);

  return (
    <section ref={sectionRef} className="min-h-[150vh] relative overflow-hidden bg-gradient-to-b from-[#0c1929] via-[#1e3a5f] to-[#2563EB]">
        ...
        >
          ...
          >
            <div className="relative">
              {/* Glowing core */}
              <div className="w-3 h-40 bg-gradient-to-b from-transparent via-blue-400 to-transparent rounded-full blur-sm" />
              {/* Outer glow */}
              <div className="absolute inset-0 w-6 h-40 -left-1.5 bg-gradient-to-b from-transparent via-blue-300/30 to-transparent rounded-full blur-md" />
              {/* Rotating ring indicators */}
              <motion.div
                className="absolute top-1/4 left-1/2 -translate-x-1/2 w-8 h-8 border-2 border-blue-400/50 rounded-full"
                animate={{ rotate: 360 }}
                transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
              />
              <motion.div
                className="absolute top-3/4 left-1/2 -translate-x-1/2 w-6 h-6 border-2 border-blue-300/40 rounded-full"
                animate={{ rotate: -360 }}
          ...
          <motion.div
            ...
            style={{ 
              ...
              scale: useTransform(smoothProgress, [0.8, 1], [0.5, 1]),
            }}
          >
      ...
      {/* Bottom gradient transition to white section */}
